...
**Stage 2 — CSS-native expressive layer (weeks 3–6).** Implement `<KineticText>` with CSS-only treatments: variable-axis choreography (`@property` + Fraunces wght/opsz/SOFT/WONK), vertical (`writing-mode`), radial (SVG `<textPath>`), scroll-driven (`animation-timeline`), background-clip masks. This is high-impact, low-bundle, and accessible by default. *Threshold:* Lighthouse/perf unaffected; all treatments have reduced-motion fallbacks.
